...
PhaserHeaderComponent::PhaserHeaderComponent()
{
    ...
    modelLabel.setFont (AtomLookAndFeel::getUIFont (AtomLookAndFeel::getSystemUIFontHeight(), juce::Font::bold));
    modelLabel.setMinimumHorizontalScale (1.0f);
    modelLabel.setAutoResizeEnabled (false);
...
void PhaserHeaderComponent::setupMeter (atom::MeterBar& meter, int barCount)
{
...
void PhaserHeaderComponent::setMeterLevels (float left, float rightL, float rightR)
{
    meterLeftLevel = left;
...
void PhaserHeaderComponent::paint (juce::Graphics& g)
{
    g.fillAll (getLookAndFeel().findColour (juce::ResizableWindow::backgroundColourId));
}

void PhaserHeaderComponent::resized()
{
    using juce::FlexBox;
    using juce::FlexItem;
    ...
    const auto modelFont = AtomLookAndFeel::getUIFont (AtomLookAndFeel::getSystemUIFontHeight(), juce::Font::bold);
    const int modelTextWidth = juce::roundToInt (modelFont.getStringWidthFloat ("JFET Phaser")) + 8;
    ...
    flexBox.items.addArray ({
        ...
        makeItem ((float) modelTextWidth, (float) h * 0.5f, modelLabel, 6, 6),
...
int PhaserHeaderComponent::getMinimumContentWidth (int heightHint)
{
    ...
    const int content = meterW + 16 + inW + 12 + gateW + 12 + modelTextWidth + 12 + cogSize + 12 + tapWidth + 12 + outW + 12 + meterW * 2 + 16;
